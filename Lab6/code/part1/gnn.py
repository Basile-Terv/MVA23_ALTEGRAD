...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        ##################
        # your code here #
        end_idx = min(i+batch_size, N_train-1)
        batch_graphs = G_train[i:end_idx]

        # adjacency matrix of the minibatch
        adj_batch = [nx.adjacency_matrix(gr) for gr in batch_graphs]
        adj_batch = sp.block_diag(adj_batch)
        adj_batch += sp.eye(adj_batch.shape[0])
        adj_batch = sparse_mx_to_torch_sparse_tensor(adj_batch).to(device)
        
        # features of nodes of the minibatch
        ...
        nb_nodes = [g.number_of_nodes() for g in batch_graphs]
        ...
        # vector that contains the class labels of the graphs
        y_batch = torch.LongTensor(y_train[i:end_idx]).to(device)
        ##################
...
for i in range(0, N_test, batch_size):
    ...
    ##################
    # your code here #
    end_idx = min(i+batch_size, N_test-1)
    batch_graphs = G_test[i:end_idx]
    ...
    # vector that indicates the graph to which each node belongs
    nb_nodes = [g.number_of_nodes() for g in batch_graphs]  # Number of nodes in each element of the batch
    idx_batch = [idx*torch.ones(total_node, dtype=torch.long) for idx, total_node in enumerate(nb_nodes)]
    idx_batch = torch.cat(idx_batch).to(device)

    # vector that contains the class labels of the graphs
    y_batch = torch.LongTensor(y_test[i:end_idx]).to(device)
    ##################
