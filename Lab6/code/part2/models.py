...
class GNN(nn.Module):
    ...
    def forward(self, x, adj, idx):
        ...
        if self.readout == 'sum':
            ...
            out = torch.zeros(torch.max(idx)+1, x.size(1), device=x.device)
            out = out.scatter_add_(0, idx, x)
            count = torch.zeros(torch.max(idx)+1, x.size(1), device=x.device) #size (nb_graphs,nb_features)
            count = count.scatter_add_(0, idx, torch.ones_like(x, device=x.device)) #counts number of nodes in each graph
