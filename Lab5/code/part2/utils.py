...
def normalize_adjacency(A):
    ...
    print('A_tilde type=',type(A_tilde))
    #D_diag=[1/np.sqrt(sum(A_tilde[i,:])) for i in range(n) ]
    D_diag=A_tilde @ np.ones(n)
    #D_diag_sqrt_inv=[1/np.sqrt(D_diag[i]) for i in range(n) ]
    D_diag_sqrt_inv=np.power(D_diag,-0.5)
    #print('D_diag_sqrt_inv=',D_diag_sqrt_inv)
    D_tilde_sqrt_inv=sp.diags(D_diag_sqrt_inv)
    #print('D_tilde_sqrt_inv=',D_tilde_sqrt_inv)
    A_normalized=D_tilde_sqrt_inv.dot(A_tilde.dot( D_tilde_sqrt_inv)) 
    ##################
    return A_normalized
